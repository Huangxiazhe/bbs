# 1. 拉代码到 /var/www/bbs-server
# 2. 执行 bash deploy.sh

set -ex
deploy_directory=/var/www/bbs-server
...
python3.6 /var/www/bbs-server/get-pip.py
...
# 建立一个软连接
ln -s -f /var/www/bbs-server/bbs.conf /etc/supervisor/conf.d/bbs.conf
# 不要再 sites-available 里面放任何东西
ln -s -f /var/www/bbs-server/bbs.nginx /etc/nginx/sites-enabled/bbs
chmod -R o+rwx /var/www/bbs-server
